...
def calc_metric(args, easy_answers, hard_answers, negative_logit,
                           queries_unflatten, query_structures,
                           predict_embeddings,
                           idxs=None):
    batch_entity_range = torch.arange(args.num_entities, dtype=torch.float).expand(
...
class KGReasoning(nn.Module):
    def __init__(self, num_entity, num_relation, hidden_dim, gamma,
        ...
        self.batch_entity_range = torch.arange(num_entity).to(torch.float).repeat(test_batch_size,
                                                                                  1).cuda() if self.use_cuda else torch.arange(
            num_entity).to(torch.float).repeat(test_batch_size, 1)  # used in test_step
        ...
        if self.geo == 'box':
            self.entity_embedding = nn.Parameter(torch.zeros(num_entity, self.entity_dim))  # centor for entities
            ...
            self.entity_embedding = nn.Parameter(torch.zeros(num_entity, self.entity_dim))  # center for entities
        elif self.geo == 'beta':
            self.entity_embedding = nn.Parameter(torch.zeros(num_entity, self.entity_dim * 2))  # alpha and beta
        ...
        if self.geo == 'box':
            self.offset_embedding = nn.Parameter(torch.zeros(num_relation, self.entity_dim))
            nn.init.uniform_(
        ...
        if all_relation_flag:
            ...
            for i in range(len(query_structure[-1])):
                ...
                if DEBUG:
                    ...
                    top10 = get_top1(embedding, self.entity_embedding)
                    print('\n', top10)
    ...
    def train_step(model, optimizer, train_iterator, args, step):
        ...
        positive_logit, negative_logit, subsampling_weight, _, _ = model(positive_sample, negative_sample,
    ...
    def test_step(model, easy_answers, hard_answers, args, test_dataloader, query_name_dict, save_result=False,
        ...
        with torch.no_grad():
            for negative_sample, queries, queries_unflatten, query_structures in tqdm(test_dataloader):
                ...
                positive_logit, negative_logit, _, idxs, predict_embeddings = model(None,
                                                                                    ...
                                                                                    batch_queries_dict,
                                                                                    batch_idxs_dict)
                tmp_logs, tmp_records = calc_metric(args, easy_answers, hard_answers, negative_logit,
                                                               queries_unflatten,
                                                               query_structures,
                                                               predict_embeddings,
                                                               idxs)
        ...
        # records_path = os.path.join(args.checkpoint_path, 'records')
        #
        # if not DEBUG:
